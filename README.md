...
Two modes behind one interface, chosen by a `delivery_provider` setting: **manual/offline** (default, flat-rate + hand-entered tracking) and **KiriminAja** (opt-in, live rates/booking/tracking via the real SDK). Both share one fulfilment state machine that reconciles with order status. See `seconds-spec.md` §19 for the full breakdown, including the documented sub-district-picker gap.
